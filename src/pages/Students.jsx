                  ...
                  <TableBody>
                    {coManagedStudents.length === 0 ? (
                      ...
                      coManagedStudents.map((student) => {
                        ...
                        const myEntry = coMentors.find(m => m.mentor_id === currentUser.id);
                        const myNet = myEntry?.net_deposit_contribution_usd || 0;
                        const primaryEntry = coMentors.find(m => m.mentor_id === student.senior_mentor_id);
                        const primaryNet = primaryEntry?.net_deposit_contribution_usd || 0;
                        const combined = coMentors.reduce((sum, cm) => sum + (cm.net_deposit_contribution_usd || 0), 0);
                        return (
                          <TableRow key={student.id} className="hover:bg-gray-50 transition-colors">
                            <TableCell className="font-medium">{student.full_name}</TableCell>
